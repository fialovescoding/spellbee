...
def _get_word_type_from_list_name(list_name: str):
    match(list_name):
        case 'new':
...
def _get_word_from_list(student_id: str, list_name: str) -> Word:
    ...
    engine = SpellBee_CrudManager()

    word_type = _get_word_type_from_list_name(list_name)

    # Get next word in list from database
    word = engine.get_next_word(student_id, word_type)

    return word
    
def _get_next_word_list_name(student_id: str):
    # Get next list to check from db
    engine = SpellBee_CrudManager()

    return engine.get_next_list(student_id)

def _set_next_word_list_name(student_id: str, next_list: str):
    engine = SpellBee_CrudManager()

    engine.set_next_list(student_id, next_list)

def move_word_to_list(student_id: str, word: Word, next_list: str):
    engine = SpellBee_CrudManager()

    word_type = _get_word_type_from_list_name(next_list)

    engine.update_word(student_id, word, word_type)


def select_next_test_word(student_id) -> Word:
    list_name: NextWordList = _get_next_word_list_name(student_id)

    if list_name is None:
        list_name = 'new'
        _set_next_word_list_name(student_id, list_name)
    else:
        list_name = list_name.next_list

    word = _get_word_from_list(student_id, list_name)
    ...
    _set_next_word_list_name(student_id, next_list)
